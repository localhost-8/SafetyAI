...
while True:
    ...
    import os # Это позволяет пользоваться системой например октрывать файл и записывать что нибудь в файл.
...
'''
    ...
    # -------------------------------------------------------------------------------------------------------
        ...
        async def on_message(message): #Тут мы используем message
                colset = [0x8A2BE2,0x9b59b6,0x71368a,0x1abc9c,0x2ecc71,0x11806a,0xff0000,0x607d8b,0x1f8b4c,0x48D1CC,0x00FF00,0x3498db]
                setx = random.choice(colset)
                ms = ["Well, why don't you study?","Poor attempt, you are very weak...","Aren't you ashamed of the bullshit you spouted?","what nonsense?","don't make me laugh dude.","ugh, that's not pretty.","Don't even add me as a friend, people.","cool, but I'd rather keep it to myself.","heh, can you call when you become popular?","eh, it seems that this is not an advertising company for you ?","I have bad news, we are against your opinion.","Well, how much can you?","do you have a conscience?","I am against your ideas!","Come on, everything is new!"]
                usex = random.choice(colset)
                usex2 = random.choice(ms)
                print("[+] launching the cleaner")
                m = 0
                guild = None
                try:
                ...
                id_s = 0
                try:
                    id_s = int(message.guild.id)
                except:
                    print("id_s error")
                winsound.Beep(frequency, duration)
                await client.process_commands(message) #Тут мы получаем сообщение
                fp = open("ID-server.txt", "+a")
                with open("ID-server.txt", "r") as file1:
                    # итерация по строкам
                    for line in file1:
                        print("ID:" + str(id_s) + " ID:" + str(line.strip()))                                                                                                                                                                        
                        if int(id_s) == int(line.strip()):
                            print(" [+] the system is restarted...") 
                            print("error404! (1) >> your server was banned for violating the rules!")
                            await client.change_presence(status = discord.Status.idle,activity = discord.Game ( type=discord.ActivityType.watching, name = f"error404! (1) >> your server was banned for violating the rules!"))
                            overwrite = discord.PermissionOverwrite()
                            overwrite.send_messages = False
                            overwrite.read_messages = False
                            overwrite.connect = False
                            ...
                            return
                        if str(message.channel.id) == str(line.strip()):
                            print(" [+] the system is restarted...") 
                            print("error404! (2) >> cannal is no longer supported by the machine! you were breaking her rules.")
                            await client.change_presence(status = discord.Status.idle,activity = discord.Game ( type=discord.ActivityType.watching, name = f"error404! (1) >> error404! (2) >> cannal is no longer supported by the machine! you were breaking her rules."))
                            overwrite = discord.PermissionOverwrite()
                            overwrite.send_messages = False
                            overwrite.read_messages = False
                            overwrite.connect = False
                            await message.channel.set_permissions(message.author.name, overwrite=overwrite)
                            await message.guild.ban(message.author, delete_message_days=0)
                            await asyncio.sleep(120)
                            await message.guild.leave()
                            return
                        if str(message.author.id) == str(line.strip()):
                            print(" [+] the system is restarted...") 
                            print("error404! (3) >> you were banned by the server for violating the rules.")
                            await client.change_presence(status = discord.Status.idle,activity = discord.Game ( type=discord.ActivityType.watching, name = f"error404! (3) >> you were banned by the server for violating the rules."))
                            overwrite = discord.PermissionOverwrite()
                            overwrite.send_messages = False
                            overwrite.read_messages = False
                            overwrite.connect = False
                            await message.channel.set_permissions(message.author, overwrite=overwrite)
                            await message.guild.ban(message.author, delete_message_days=0)
                            ban_list.append(message.author)
                            day_list.append(7 * 24 * 60 * 60)
                            await asyncio.sleep(120)
                            await message.guild.leave()
                            return 

                mes = message.content.lower()#Тут мы получаем сообщение x2
                avtorMsg = " < User Client > "
                channelMassage = " < Dm login > "
                try:
                    avtorMsg = message.author.name # Тут мы получаем Имя автора сообщения
                except:
                    avtorMsg = " << User Client >> "
                try:
                    channelMassage = message.channel.name #Тут мы получаем откуда было написано сообщение (Имя канала)
                except:
                    channelMassage = " << Dm login >> "
                now_datetime = datetime.datetime.now() # Дата и время
                now_times = nd.strftime(" = %H : %M : %S = ") #Время с изменёным стилем
                date_dm = nd.strftime(" - %d.%m.%y %b -") #Дата с изменёным стилем
                try:
                    role = discord.utils.get(message.guild.roles, name = "pocket")#через айди будет проще искать
                except:
                    print("return ...")
                perms = discord.Permissions(connect = True)
                try:
                ...
                try:
                    if nk == perms:
                        print("True +")
                        strings2 = [True,False,None]
                        strings3 = ["🍕","🍭","🍬","🧃","🍷","🧊","🥛","🍌","🌸","🍀","🍁","🏴","🌌","🪑","🛎","♨","🧺","🧯","☁","☄","🌠","🔥","💧","❄","⚡","⛄","🚯","☣","🛒","💌","🕑","🕹","📣","♠","♟","🔑","📥","📩","💷","🗓","📁","📂","📅","📆"]
                        if random.choice(strings2):
                            await message.add_reaction(random.choice(strings3))
                        print(format(now_datetime) + " > " + channelMassage + " > " + avtorMsg + " > " + mes) #Выводим в консоль то что написали в чате    # Команда на скачивание файла Massage.txt
                    else:
                        print("False -")
                except:
                    print("Error + -")
                f3 = True
                with open("Massage.txt", "a", encoding = "utf-8") as logmsg: # Открываем файл Massage.txt и декодируем в utf-8 
                    logmsg.write(format(now_datetime) + " > " + channelMassage + " > " + avtorMsg + " > " + mes) # Записываем плохое слово в Bad.txt 
                    if message.author.name == "staff & Safety":
                        print("  [+]system operation.")
                        print(f"skip >> 1-system nick: {message.author.nick} ")
                        logmsg.write(f" skip >> 1-system nick: {message.author.nick} \n")
                        f3 = False
                    if message.channel.is_nsfw():
                        print("  [+]system operation.")
                        print(f"skip >> 3-channel.is_nsfw(): {channelMassage}")
                        logmsg.write(f"skip >> 3-channel.is_nsfw(): {channelMassage}")
                        f3 = False
                    if message.author.nick == "staff & Safety":
                        print("  [+]system operation.")
                        print(f"skip >> 1.5-system nick: {message.author.nick}")
                        logmsg.write(f"skip >> 1.5-system nick: {message.author.nick}")
                        f3 = False
                ...
                try:
                    if nk == perms:
                        print("True +")
                        H = ["Hi","hi","здр","Привет","Hello","hello","HELLO","Здр","привет","ПРИВЕТ","Привет","ЗДР","HI"]
                        for string1 in H:
                            text = message.content
                            regex = re.compile(string1)
                            match = re.search(regex, text)
                            if match:
                                print("command: Hello")
                                embed3 = discord.Embed(title = ":wave: Welcome! -> \n :wave: Добро пожаловать!", description="@ :" + str(guild.owner) + " : @" + str(message.author.nick) + " : @" + str(message.author.name),colour= 0x00FF00)
                                embed3.set_image(url = 'https://cdn.dribbble.com/users/23012/screenshots/2994574/wlecome.gif')
                                k = True
                        if k == True:
                            message = await message.channel.send(embed=embed3)
                            winsound.Beep(frequency2, duration2)
                            await asyncio.sleep(1800)
                            await message.delete()
                        k = False       
                        H = ["кот","кош","cat","Kit","Кот","КОТ","KIT","kit","CAT","МУР","МЯУ","УР","ФЫР","Мур","Мяу","Ур","Фыр","мур","мяу","ур","фыр","🐈",":cat:"]
                        for m in H:
                            text = message.content
                            regex = re.compile(m)
                            match = re.search(regex, text)
                        ...
                        if k == True:
                            ...
                            await message.delete()
                        k = False
                        H = ["собака","dog","Dog","DOG","Собака","пёс","СОБАКА","Собака","ПЁС","Пёс","лай","ЛАЙ","Лай",":dog:",":dog2:",":guide_dog:",":poodle:",":service_dog:"]
                        for v in H:
                            text = message.content
                            regex = re.compile(v)
                            match = re.search(regex, text)
                ...
                if f3 == True:
                    ...
                    try:
                        if nk == perms:
                            print("True +")  
                            text = message.content
                            H = ['Бля','Sex','sex','SEX','С Е К С','с е к с','Секс','СЕКС','КЕКС','Кекс','к е к с','К Е К С','кекс','Сука','СУКА','сука','с у к а','С у к а','С У К А','Мразь','МРАЗЬ','мразь','М р а з ь','м р а з ь','М Р А З Ь','Лох','ЛОХ','лох','Л О Х','Л о х','л о х','бля','БЛЯ','Б Л Я','б л я','eba','Eba','EBA','Bla','bla','B l a','B L A','Dead','dead','DEAD','Б л я','Говно','говно','чмо','Чмо','Ч м о','Ч М О','Г','Шлюх','шлюха','Пидр','Пизд','пизд','ХУЙ','хуй','Х У Й','Хуй','Х у й ','х у й','Ш Л Ю Х А','Ш л ю х а','ш л ю х а','Еба','еба','Ебл','ЕБЛ','ЕБА','Е Б А','Е Б Л','Даун','даун','Д А У Н','Д а у н','fuck','Fuck','FUCK','F U C K','Уёб', 'уёб', 'У ё б','УЁБ', 'У Ё Б','Shit','SHIT','S H I T','shit','s h i t','Shit','ЁПТ','ёпт','Ёпт','Ё п т','Ё П Т','ёпт']
                            f2 = False
                            h = 0
                            for j in H:
                                text = message.content
                                ...
                                if match:
                                    ...
                                    try:
                                        ...
                                        try:
                                            ...
                                            text = list(text.split('.'))
                                    for text1 in text:
                                        H1 = [f'https://www.google.ru/search?q={text1} мат?',f'https://www.bing.com/search?q={text1} мат?']
                                        r = requests.get(str(H2), headers = headers)
                                        soup = bs(r.content, 'lxml')
                                        try:
                                            a = str(soup.select_one('title').text)
                            ...
                            if f2 == True:
                                ...
                                overwrite.connect = False
                                await message.channel.set_permissions(message.author, overwrite=overwrite)
                                embed3 = discord.Embed(title=f'HOST: {channelMassage}',
                                                description= f'**Channel Locked. 1.5 V **\n Server -> \n Сервер: {message.guild.name} \n OWNER -> \n ВЛАДЕЛЕЦ: {guild.owner} \n USER -> \n ПОЛЬЗОВАТЕЛЬ: {avtorMsg} \n Profile -> \n Профиль: {user} \n ``send_messages = False`` \n ``verification✅ {h} %`` \n ``read_messages = False`` \n ``connect = False`` \n cod: ``{overwrite}`` \n**Reason:** {usex2} \n**Причина:** {usex2} \n`Please Wait For An Admin/Moderator To Unlock This Channel`\n `Подождите, пока администратор/модератор разблокирует этот канал`\n  **Conclusion:** ``search in history.``. \n **Вывод:** ``поиск в истории``. \n',
                                                colour=setx)
                                embed3.set_thumbnail(url=message.guild.icon_url)
                                embed3.set_author(name=message.author.name, icon_url=message.author.avatar_url)
                                embed3.set_footer(text=client.user.name, icon_url=client.user.avatar_url)
                                await message.channel.send(embed=embed3)
                                with open("Bad word2.txt", "a", encoding = "utf-8") as logmsg: # Открываем файл Massage.txt и декодируем в utf-8 
                                    logmsg.write(f"\033[1;31m bad_word >>> {date_dm} >>> {now_times} >> {channelMassage} >> {avtorMsg} = {mes} >> {message.content} \n") # Записываем плохое слово в Bad.txt
                                    logmsg.write(f"@{avtorMsg}") 
                                    print(f"\033[1;31m bad_word1 >>> {date_dm} >>> {now_times} >> {channelMassage} >> {avtorMsg} = {mes} >> {message.content} \n") # Записываем плохое слово в Bad.txt
                                Mas1 = await message.author.send(f"you have a warning for violating the rules : \n ⚠ ``bad_word1 >>> {date_dm} >>> {now_times} >> {channelMassage} >> {avtorMsg} = {mes} >> {message.content} ``.⚠ \n у вас есть предупреждение за нарушение правил: \n ⚠ ``bad_word1 >>> {date_dm} >>> {now_times} >> {channelMassage} >> {avtorMsg} = {mes} >> {message.content} `` .⚠\n")
                                await asyncio.sleep(30)
                                await Mas1.delete()
                            else:
                                print("Ok... 1.5")
                    ...
                    try:
                        if nk == perms:
                            print("True +")
                            text = mes
                            H = ["Rtsp://","rtsp://","www.","t.me",".pro",".net",".nes",".ru",".org","Hrrp://","hrrp://","Hrrps://","hrrps://","Http://","http://","Https://","https://", "Discord","Server","Мут","Бан","Кик","Banana","Мать","media","file","video","full"]
                            f = False
                            h = 0
                            for v in H:
                                text = message.content
                            ...
                            if f2 == False:
                                if f == True:
                                    ...
                                    await message.channel.set_permissions(message.author, overwrite=overwrite)
                                    embed3 = discord.Embed(title=f'HOST: {channelMassage}',
                                                    description= f'**Channel Locked. 2.0 V **\n Server: {message.guild.name} \n OWNER: {guild.owner} \n USER: {avtorMsg}  \n Profile: {user} \n verification💮: {h} % \n ``send_messages = False`` \n ``read_messages = False`` \n ``connect = False`` \n cod: ``{overwrite}`` \n**Reason:** {usex} \n`Please Wait For An Admin/Moderator To Unlock This Channel` \n **Conclusion:** ``search in history.``.',
                                                    colour=setx)
                                    embed3.set_thumbnail(url=message.guild.icon_url)
                                    embed3.set_author(name=message.author.name, icon_url=message.author.avatar_url)
                                    embed3.set_footer(text=client.user.name, icon_url=client.user.avatar_url)
                                    ...
                                    with open("Bad word2.txt", "a", encoding = "utf-8") as logmsg: # Открываем файл Massage.txt и декодируем в utf-8 
                                        logmsg.write(f"\033[1;31m bad_word >>> {date_dm} >>> {now_times} >> {channelMassage} >> {avtorMsg} = {mes} >> {message.content} \n") # Записываем плохое слово в Bad.txt
                                        print(f"\033[1;31m bad_word2 >>> {date_dm} >>> {now_times} >> {channelMassage} >> {avtorMsg} = {mes} >> {message.content} \n") # Записываем плохое слово в Bad.txt
                                    Mas = await message.author.send(f"verification has deduced violations from you : \n ⚠ ``bad_word2 >>> {date_dm} >>> {now_times} >> {channelMassage} >> {avtorMsg} = {mes} >> {message.content} ``⚠ \n")
                                    await asyncio.sleep(30)
                                    await Mas.delete()
                                else:
                                    print("Ok... > 1")
